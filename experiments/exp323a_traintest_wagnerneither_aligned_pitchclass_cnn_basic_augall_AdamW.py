...
path_data = os.path.join(path_data_basedir, 'Wagner_Ring_allperf', 'hcqt_hs512_o6_h5_s1')
path_annot = os.path.join(path_data_basedir, 'Wagner_Ring_allperf', 'pitchclass_hs512_nooverl')

test_dataset_list = ['Wagner_Ring', 'Wagner_Ring']
path_test_data_list = [os.path.join(path_data_basedir, ds_name, 'hcqt_hs512_o6_h5_s1') for ds_name in test_dataset_list]
path_test_annot_list = [os.path.join(path_data_basedir, ds_name, 'pitchclass_hs512_nooverl') for ds_name in ['Wagner_Ring']]
path_test_annot_list.extend([os.path.join(path_data_basedir, ds_name, 'pitchclass_hs512_nooverl_fullscore') for ds_name in ['Wagner_Ring']])
...
train_versions = ['Boulez', 'Bohm', 'Furtwangler', 'Janowski', 'Keilberth', 'Krauss', 'Levine', 'Neuhold', 'Swarowsky', 'Thielemann']
val_versions = ['Sawallisch', 'Solti', 'Weigle']
test_versions = ['BarenboimKupfer', 'Karajan', 'Haitink']
exclude_versions = ['MEMBRAN2013']

train_acts = ['WWV086A', 'WWV086C-1', 'WWV086C-3', 'WWV086D-1', 'WWV086D-2', 'WWV086D-3']
val_acts = ['WWV086C-2', 'WWV086D-0']
test_acts = ['WWV086B-1']
...
if do_train:
    for fn in os.listdir(path_data):
        if any(train_version in fn for train_version in train_versions) and \
            any(train_act in fn for train_act in train_acts) and \
            not any(excl_vers in fn for excl_vers in exclude_versions) and \
            not np.mod(rand_counter, 3)==1:
        ...
        if do_val:
            if any(val_version in fn for val_version in val_versions) and \
                any(val_act in fn for val_act in val_acts) and \
                not any(excl_vers in fn for excl_vers in exclude_versions):
    ...
    for test_dataset in test_dataset_list:
        ...
        for fn in os.listdir(path_data):
            if any(test_version in fn for test_version in test_versions) and \
                any(test_act in fn for test_act in test_acts):
                ...
                del inputs, targets, inputs_context, targets_context
                gc.collect()
