...
dataset_list = ['MusicNet', 'Schubert_Winterreise', 'Bach10', 'PHENICX-Anechoic', 'ChoralSingingDataset', 'TRIOS']
test_dataset_list = ['Wagner_Ring_allperf', 'Wagner_Ring']
...
path_test_annot_list.extend([os.path.join(path_data_basedir, ds_name, 'pitchclass_hs512_noover_fullscore') for ds_name in ['Wagner_Ring']])
...
if do_val:
    ...
    val_versions = ['2303_', '1819_', '2382_', '1759_', '2106_', '2191_', '2298_', '2416_', '2556_', '2629_']    # as in paper

test_and_val_versions = val_versions
...
train_songs = ['D911-01', 'D911-02', 'D911-03', 'D911-04', 'D911-05', 'D911-06', 'D911-07', \
    'D911-08', 'D911-09', 'D911-10', 'D911-11', 'D911-12', 'D911-13', 'D911-14', 'D911-15', 'D911-16']
train_versions = ['AL98', 'FI55', 'FI80', 'OL06', 'QU98', 'FI66', 'TR99']

val_versions = ['D911-17_HU33', 'D911-18_HU33', 'D911-19_HU33', 'D911-20_HU33', 'D911-21_HU33', \
...
#### Bach10 ###############################
train_versions = ['01-', '02-', '03-', '04-', '05-', '06-']
val_versions = ['07-', '08-', '09-', '10-']
...
if do_train:
    ...
    if do_val:
        val_set_phenicx = torch.utils.data.ConcatDataset(all_val_sets)
        logging.info('   ######## Validation set PHENICX-Anechoic generated, length ' + str(len(val_set_phenicx)) + ' ######')

#### ChoralSingingDataset ###############################
train_versions = ['CSD_Traditional_ElRossinyol', 'CSD_Guerrero_NinoDios']
val_versions = ['CSD_Bruckner_LocusIste']
...
#### TRIOS ###############################
train_versions = ['mozart', 'schubert', 'take_five']
val_versions = ['brahms', 'lussier']
...
path_data = path_data_list[5]
path_annot = path_annot_list[5]

train_dataset_params['stride'] = 3
val_dataset_params['stride'] = 3
...
if do_train:
    ...
    train_set_trios = torch.utils.data.ConcatDataset(all_train_sets)
    logging.info('   ######## Training set TRIOS generated, length ' + str(len(train_set_trios)) + ' ######')

    if do_val:
        val_set_trios = torch.utils.data.ConcatDataset(all_val_sets)
        logging.info('   ######## Validation set TRIOS generated, length ' + str(len(val_set_trios)) + ' ######')
    ...
    for test_dataset in test_dataset_list:
        ...
        for fn in os.listdir(path_data):
            if any(test_version in fn for test_version in test_versions):
                ...
                if k_testdata==1:
                    np.save(os.path.join(dir_predictions, fn[:-4]+'_fullscore.npy'), pred)
        ...
        if k_testdata==1:
            logging.info(' ### Dataset ' + test_dataset + '_fullscore tested. Individual results: ################################ \n')
