...
from libdl.nn_models import simple_u_net_polyphony_classif_softmax
...
# Log criterion, optimizer, and scheduler ######################################
    ...
    logging.info('Criterion 1 :  ' + criterion1.__class__.__name__)
    logging.info('Criterion 2 :  ' + criterion2.__class__.__name__)
...
# train_versions = ['AL98', 'FI55', 'FI80', 'OL06', 'QU98']
    ...
    # '2629_','2632_','2633_']   # randomly selected 33 (no, 27!)
    val_versions = ['1729_','1733_','1755_','1756_','1765_','1766_','1805_','1807_','1811_', \
    '1932_','1933_','2081_','2082_','2083_','2157_','2158_','2167_', \
    '2221_','2222_','2289_','2315_','2318_','2341_','2342_','2480_', \
    '2632_','2633_']   # randomly selected 27
test_pieces = ['_WTKI', 'Mozart_K375_Serenadein', 'Beethoven_OP130_StringQuartet', 'Schubert_D958_PianoSonata', \
                'Haydn_OP64NO5_QuartetNo', 'Bach_BWV1006_ViolinPartita', 'Bach_BWV1010_CelloSuite', \
                'Beethoven_OP71_Sextetin', 'Beethoven_OP109_PianoSonata', 'Beethoven_OP96_ViolinSonata']
test_versions_small = ['2303_', '1819_', '2382_']    # as in Thickstun paper
test_versions1 = ['2303_', '1819_', '2382_', '1759_', '2106_', '2191_', '2298_', '2416_', '2556_', '2629_']    # as in our paper (with "typo")
test_versions2 = ['2303_', '1819_', '2382_', '1759_', '2106_', '2191_', '2298_', '2416_', '2556_', '2628_']    # as in Wu paper
test_versions3 = ['2302_', '1818_', '2383_', '1758_', '2105_', '2186_', '2293_', '2415_', '2557_', '2627_']    # mainly slow mvmts.
test_versions4 = ['2310_', '1817_', '2381_', '1757_', '2104_', '2186_', '2296_', '2417_', '2555_', '2626_']    # mainly fast mvmts.

all_test_subsets = [test_versions1, test_versions_small, test_versions2, test_versions3, test_versions4, test_pieces]
all_test_subset_names = ['large test set (10 files) own with "typo"', 'small test set (3 files), first 90s', \
                        'large test set (10 files) as Wu et al.', 'large test set (10 files) alternative1, slow mvmts.',
                        'large test set (10 files) alternative2, fast mvmts.', 'full test sets (all mvmts.)']

test_and_val_versions = test_pieces + test_versions_small + test_versions1 + test_versions2 + test_versions3 + test_versions4 + val_versions
...
#### START TRAINING ############################################################
    ...
    for epoch in range(max_epochs):
        accum_loss, accum_loss1, accum_loss2, n_batches = 0, 0, 0, 0
        for local_batch, local_labels in train_loader:
            # Transfer to GPU
            local_batch, local_labels = local_batch.to(device), local_labels.to(device)

            # Model computations
            y_pred, n_pred = model(local_batch)
            n_target = torch.sum(local_labels, dim=-1, keepdims=True).long().squeeze(3).to(device)
            loss1 = criterion1(y_pred, local_labels)
            loss2 = criterion2(n_pred, n_target)/25.0
            loss = loss1 + loss2
        ...
        train_loss1 = accum_loss1/n_batches
        train_loss2 = accum_loss2/n_batches

        if do_val:
            accum_val_loss, accum_val_loss1, accum_val_loss2, n_val = 0, 0, 0, 0
            for local_batch, local_labels in val_loader:
                ...
                accum_val_loss1 += loss1.item()
                accum_val_loss2 += loss2.item()
                n_val += 1
            val_loss = accum_val_loss/n_val
            val_loss1 = accum_val_loss1/n_val
            val_loss2 = accum_val_loss2/n_val
        ...
        if sp['use_scheduler'] and sp['name']=='LambdaLR' and do_val:
            ...
            logging.info('Epoch #' + str(epoch) + ' finished. Train Loss: ' + "{:.4f}".format(train_loss) + ' (' + "{:.4f}".format(train_loss1) + ', ' + "{:.4f}".format(train_loss2) + \
            '), Val Loss: ' + "{:.4f}".format(val_loss) + ' (' + "{:.4f}".format(val_loss1) + ', ' + "{:.4f}".format(val_loss2) + ' with lr: ' + "{:.5f}".format(optimizer.param_groups[0]['lr']))
...
if do_test:
    ...
    for test_subset in range(6):
        test_versions = all_test_subsets[test_subset]
        test_subset_name = all_test_subset_names[test_subset]
